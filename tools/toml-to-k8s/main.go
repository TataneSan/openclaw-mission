...
func tomlToK8sYAML(data map[string]any, kind string) string {
...
var rootCmd = &cobra.Command{
	Use:   "toml-to-k8s",
	Short: "Convert TOML to Kubernetes YAML manifests",
	Long: `toml-to-k8s reads a TOML file and generates Kubernetes YAML manifests.

Examples:
  toml-to-k8s config.toml
  toml-to-k8s -k Deployment config.toml
  toml-to-k8s -o output.yaml config.toml`,
	RunE: func(cmd *cobra.Command, args []string) error {
		if len(args) != 1 {
			return fmt.Errorf("exactly one TOML file path required")
		}
		...
		if _, err := toml.DecodeFile(args[0], &data); err != nil {
		...
		for _, key := range sortedKeys(data) {
			if nested, ok := data[key].(map[string]any); ok {
				outputs = append(outputs, tomlToK8sYAML(nested, kind))
			} else {
				outputs = append(outputs, tomlToK8sYAML(map[string]any{"name": key, "value": data[key]}, kind))
			}
		}
